...
def show_execution_details(cluster_name, execution_id, resource_group, **kwargs):
...
def delete(cluster_name, name, resource_group, **kwargs):
    return call_az("az hdinsight script-action delete", locals())


def execute(cluster_name, name, resource_group, roles, script_uri, persist_on_success=None, script_parameters=None, **kwargs):
    '''
...
def list_execution_history(cluster_name, resource_group, **kwargs):
    return call_az("az hdinsight script-action list-execution-history", locals())


def promote(cluster_name, execution_id, resource_group, **kwargs):
