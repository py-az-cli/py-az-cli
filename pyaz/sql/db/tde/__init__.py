...
def set(database, resource_group, server, status, **kwargs):
    '''
...
def show(database, resource_group, server, **kwargs):
    return call_az("az sql db tde show", locals())


def list_activity(database, resource_group, server, **kwargs):
